Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\shiftavenue.GraphAutomation").Path
	$manifest = ((Get-Content "$moduleRoot\shiftavenue.GraphAutomation.psd1") -join "`n") | Invoke-Expression
